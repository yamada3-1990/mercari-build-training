...
### ■画像が上手く格納できなかった点について
Gemini
>* r.FormValue("image") は、フォームの値を文字列として取得します。  
>* []byte(r.FormValue("image")) は、取得した文字列をバイト列に変換します。  
>* これは、画像をBase64エンコードなどの文字列として渡している場合以外、バイナリデータである画像ファイルを正しく処理できません。  
>* 画像ファイルはバイナリデータであり、文字列として扱うとデータが破損します。 

FormFile("image")にすべきところをFormValue("image")にしてたことが問題っぽい(文字列として扱ってしまっていた)
